...
/// <summary>
/// Tier 1 unit tests for <see cref="SetPasswordModel.InputModel.NewPassword"/>'s validation
...
/// Mirrors <c>ChangePasswordModelInputTests</c> -- SetPassword's NewPassword rule is identical.
/// </summary>
public class SetPasswordModelInputTests
{
    private static bool IsNewPasswordValid(string? candidate)
    {
        var model = new SetPasswordModel.InputModel();
        var context = new ValidationContext(model) { MemberName = nameof(SetPasswordModel.InputModel.NewPassword) };
