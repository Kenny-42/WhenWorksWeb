...
- **`[RegularExpression]` alone never rejects an empty value — pair it with `[Required]` or `StringLength(..., MinimumLength: ...)`.** By .NET design, `RegularExpressionAttribute.IsValid` treats `null`/`""` as automatically valid, deferring "was something entered at all" to a separate attribute. `Participant.Color`, `EventMessage.SenderColor`, and `ApplicationUser.Color` all pair `[RegularExpression(HexColorPattern)]` with `[StringLength(HexColorLength)]` — that `StringLength` must set `MinimumLength = HexColorLength` (not just the max), or an empty string silently passes both attributes. This was a real gap until fixed on all three properties; every `[RegularExpression]` usage in this codebase should be checked for the same pairing when adding a new one.
