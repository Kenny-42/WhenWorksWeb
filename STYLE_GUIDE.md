...
### `--gradient-page`
The site's signature background — a five-stop animated diagonal gradient (hot pink → coral → salmon → light orange → pale gold), applied at the `body` level so it's consistent across every page. Drifts slowly via a JS-driven canvas (see site.js), disabled under `prefers-reduced-motion`.
...
### `--color-text-strong`
A dark, dusty gray (`#514c54`) — the default dark text color on light surfaces: accent fills (yellow/blue/green pills, GO button interiors), white-glass tags/cards (`.ww-hero-badge`, `.ww-feature-card`), and the `.text-on-gradient` utility.
...
This is unrelated to `#ww-spotlight` (`site.js`/`site.css`) — the ambient, page-wide, cursor-tracking glow (screen/plus-lighter blended, tinted to whatever's under it via each element's `--card-glow-rgb`). The page-wide spotlight is the intended, permanent design, not a placeholder; a couple of alternatives (a "gooey" cursor-shadow layer, and a version scoped to Home's feature cards only) were tried and reverted per developer direction. The three feature cards read as more "specified"/vivid than the plain page background because they carry their own `--card-glow-rgb`, which this same spotlight always reads from whatever's under the cursor — not through any card-specific code.
